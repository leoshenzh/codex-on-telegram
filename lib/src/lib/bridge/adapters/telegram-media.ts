/**
 * Telegram Media — download and process files from Telegram messages.
 *
 * Handles photo[] size selection, document validation, file download via Bot API,
 * and base64 conversion. Image attachments can be passed directly to model vision;
 * non-image documents are saved by the conversation engine and exposed as local paths.
 */
...
/** Default max inbound file size in bytes (20 MB). */
...
/**
 * Get the configured max inbound file size in bytes.
 */
function getMaxFileSize(): number {
  const setting = getBridgeContext().store.getSetting('bridge_telegram_max_file_size')
    || getBridgeContext().store.getSetting('bridge_telegram_max_image_size');
  if (setting) {
...
export async function downloadDocumentImage(
  ...
  return downloadDocumentFile(botToken, doc, messageId);
}

/**
 * Download any Telegram document within the configured size limit.
 */
export async function downloadDocumentFile(
  ...
  return downloadFileById(botToken, doc.file_id, messageId, {
    ...
    mimeType: doc.mime_type || inferMimeType(doc.file_name || ''),
  });
}
...
async function downloadFileById(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const mime = opts?.mimeType || inferMimeType(filePath) || (filePath.startsWith('photos/') ? 'image/jpeg' : 'application/octet-stream');

      // Step 5: Convert to base64 and build FileAttachment
      const base64 = buffer.toString('base64');
      const fileName = opts?.fileName || filePath.split('/').pop() || `file_${messageId}`;
...
function formatSizeError(actualBytes: number, limitBytes: number): string {
  ...
  return `File too large (${actualMB} MB, limit ${limitMB} MB). Please send a smaller file.`;
}
