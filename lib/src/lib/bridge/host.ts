...
import type { ChannelBinding, ChannelType, SessionSource } from './types.js';
...
/** Runtime status persisted by hosts that support live session diagnostics. */
...
export interface LLMProvider {
  /**
   * Optional startup probe. Should throw when the provider is definitely not
   * usable in the current environment.
   */
  preflight?(): Promise<void>;
  preflightCheck?(): Promise<{ ok: boolean; error?: string }>;
