...
describe('catch block error suppression logic', () => {
  ...
  it('result received + exit code → should suppress (transport noise)', () => {
    ...
    const shouldSuppress = state.hasReceivedResult && isPostResultTransportError(errorMsg);
    assert.equal(shouldSuppress, true);
  });

  it('result received + socket hang up → should suppress (transport noise)', () => {
    const state: StreamState = { hasReceivedResult: true, hasStreamedText: true, lastAssistantText: '' };
  ...
  it('assistant auth error wins over post-result transport suppression', () => {
    const state: StreamState = {
      hasReceivedResult: true,
      hasStreamedText: false,
      lastAssistantText: 'Not logged in · Please run /login',
    };
    const errorMsg = 'Claude Code process exited with code 1';

    const assistantAuthMessage = authUserMessage(classifyAuthError(state.lastAssistantText));
    const shouldSuppress = !assistantAuthMessage && state.hasReceivedResult && isPostResultTransportError(errorMsg);

    assert.match(assistantAuthMessage || '', /Claude CLI is not logged in/);
  ...
  it('streaming + result + exit code → should suppress', () => {
    ...
    const shouldSuppress = state.hasReceivedResult && isPostResultTransportError('Claude Code process exited with code 1');
    assert.equal(shouldSuppress, true);
  });
});
