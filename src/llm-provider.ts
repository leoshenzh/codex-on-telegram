...
export function authUserMessage(kind: AuthErrorKind): string | null {
  if (kind === 'cli') return CLI_AUTH_USER_MESSAGE;
  if (kind === 'api') return API_AUTH_USER_MESSAGE;
  return null;
}
...
export function isPostResultTransportError(message: string): boolean {
...
export class SDKLLMProvider implements LLMProvider {
  ...
  async preflight(): Promise<void> {
    ...
    const check = preflightCheck(this.cliPath);
    if (!check.ok) {
      throw new Error(check.error || 'Claude CLI preflight failed');
    }
  }
      ...
      start(controller) {
        (async () => {
          ...
          try {
            ...
            // ── Case 1: Recognised business error in assistant text ──
            // Claude Code can emit an assistant error such as "Not logged in"
            // and then exit with code 1 after a result frame. Surface the
            // actionable auth error before treating the exit as transport noise.
            const assistantAuthKind = state.lastAssistantText
              ? classifyAuthError(state.lastAssistantText)
              : false;
            const assistantAuthMessage = authUserMessage(assistantAuthKind);
            if (assistantAuthMessage) {
              controller.enqueue(sseEvent('error', assistantAuthMessage));
              controller.close();
              return;
            }

            // ── Case 2: Result already received ──
            // The SDK delivered a proper result (success or structured error).
            // A trailing transport teardown error is noise.
            if (state.hasReceivedResult && isPostResultTransportError(message)) {
            ...
            } else if (isPostResultTransportError(message)) {
